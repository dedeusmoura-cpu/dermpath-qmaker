...
export default function Home() {
  ...
  if (mode === "new") return <main className={styles.shell}><AppHeader home language={language} setLanguage={setLanguage} onHistory={() => setMode("home")} /><section className={styles.card}><header><div><span className={styles.kicker}>{editingId ? (en ? "EDIT QUIZ" : "EDITAR QUIZ") : (en ? "CREATE QUIZ" : "CRIAR QUIZ")}</span><h1>{editingId ? (en ? "Edit quiz" : "Editar quiz") : (en ? "Create a new quiz" : "Criar um novo quiz")}</h1></div>{!editingId && <button type="button" className={styles.back} onClick={() => { load(); setMode("challenge-new"); }}>{en ? "🏆 Trophy challenge" : "🏆 Desafio Troféu"}</button>}</header><form className={styles.form} onSubmit={create}>
