...
export default function Home() {
  ...
  const [challengeList, setChallengeList] = useState<{ id: number; title: string; createdAt: string }[]>([]);
  ...
  const emptyForm = { title: "", stem: "", imageUrl: "", kind: "choice" as "choice" | "cloud" | "open", options: ["", "", "", ""], correctAnswer: 0 };
  function startCreate() { setForm(emptyForm); setEditingId(null); setMessage(""); setMode("new"); }
  async function startEdit(questionId: number) {
    const response = await fetch(`/api/questions/${questionId}`);
    const data = await response.json();
    if (!response.ok) return alert(data.error ?? (en ? "Could not load the quiz." : "Não foi possível carregar o quiz."));
    const loaded = data.question as Question;
    setForm({ title: loaded.title, stem: loaded.stem, imageUrl: loaded.imageUrl ?? "", kind: loaded.kind, options: loaded.kind === "choice" ? [...loaded.options, "", "", "", ""].slice(0, Math.max(4, loaded.options.length)) : ["", "", "", ""], correctAnswer: loaded.correctAnswer });
  ...
  async function create(event: FormEvent) {
    ...
    if (editingId) {
      const response = await teacherFetch(`/api/questions/${editingId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }, en);
      const data = await response.json().catch(() => null);
      if (!response.ok) return setMessage(data?.error ?? (en ? "Could not save the changes." : "Não foi possível salvar as alterações."));
      location.href = `?q=${editingId}&painel=1`;
      return;
    }
    const response = await fetch("/api/questions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const data = await response.json(); if (!response.ok) return setMessage(data.error); location.href = `?q=${data.question.id}&painel=1`;
  }
  ...
  function startCreateChallenge() { load(); setChallengeTitle(""); setChallengeQuestionIds([]); setEditingChallengeId(null); setChallengeMessage(""); setMode("challenge-new"); }
  async function startEditChallenge(challengeId: number) {
    load();
    const response = await fetch(`/api/challenges/${challengeId}`);
    const data = await response.json();
    if (!response.ok) return alert(data.error ?? (en ? "Could not load the challenge." : "Não foi possível carregar o desafio."));
    setChallengeTitle(data.challenge.title);
    setChallengeQuestionIds(data.challenge.questions.map((question: ChallengeQuestion) => question.id));
    setEditingChallengeId(challengeId);
    setChallengeMessage("");
    setMode("challenge-new");
  }
  async function deleteChallenge(challengeId: number, title: string) {
    if (!confirm(en ? `Delete "${title}"? This removes its players and scores too.` : `Excluir "${title}"? Isso também remove os jogadores e pontuações dele.`)) return;
    const response = await teacherFetch(`/api/challenges/${challengeId}`, { method: "DELETE" }, en);
    if (response.ok) { setChallengeList((current) => current.filter((item) => item.id !== challengeId)); return; }
    const data = await response.json().catch(() => null);
    alert(data?.error ?? (en ? "Could not delete the challenge." : "Não foi possível excluir o desafio."));
  }
  async function createChallenge(event: FormEvent) {
    event.preventDefault();
    if (editingChallengeId) {
      const response = await teacherFetch(`/api/challenges/${editingChallengeId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: challengeTitle, questionIds: challengeQuestionIds }) }, en);
      const data = await response.json().catch(() => null);
      if (!response.ok) return setChallengeMessage(data?.error ?? (en ? "Could not save the changes." : "Não foi possível salvar as alterações."));
      location.href = `?trofeu=${editingChallengeId}&painel=1`;
      return;
    }
    const response = await fetch("/api/challenges", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: challengeTitle, questionIds: challengeQuestionIds }) }); const data = await response.json(); if (!response.ok) return setChallengeMessage(data.error); location.href = `?trofeu=${data.challenge.id}&painel=1`;
  }
  ...
  async function deleteQuestion(questionId: number, title: string) {
    ...
    if (response.ok) { setQuestions((current) => current.filter((item) => item.id !== questionId)); return; }
    const data = await response.json().catch(() => null);
    alert(data?.error ?? (en ? "Could not delete the quiz." : "Não foi possível excluir o quiz."));
  }
  ...
  return <main className={styles.shell}><AppHeader home language={language} setLanguage={setLanguage} /><section className={styles.dashboard}><header><div><span className={styles.kicker}>{en ? "QUIZ HISTORY" : "HISTÓRICO DE QUIZZES"}</span><h1>{en ? "Previous quizzes." : "Quizzes anteriores."}</h1><p>{en ? "Open a teacher panel and its QR Code." : "Abra o painel do professor e o QR Code de cada quiz."}</p></div><button className={styles.primary} onClick={startCreate}>{en ? "+ Create quiz" : "+ Criar quiz"}</button></header><div className={styles.list}>{questions.length ? questions.map((item) => { const responseCount = item.responseCount ?? 0; const type = item.kind === "cloud" ? (en ? "WORD CLOUD" : "NUVEM DE PALAVRAS") : item.kind === "open" ? (en ? "OPEN RESPONSE" : "RESPOSTA ABERTA") : (en ? "MULTIPLE CHOICE" : "MÚLTIPLA ESCOLHA"); const answerLabel = en ? (responseCount === 1 ? "answer" : "answers") : (responseCount === 1 ? "resposta" : "respostas"); return <div className={styles.listItem} key={item.id}><a href={`?q=${item.id}&painel=1`}><div className={styles.questionCardMain}><span className={styles.questionType}>{type}</span><b>{item.title}</b><small>{en ? "Quiz" : "Questão"} {item.id} · {responseCount} {answerLabel}</small></div><span className={styles.cardArrow} aria-hidden="true">→</span></a><div className={styles.cardActions}><button type="button" className={styles.editQuestion} onClick={() => startEdit(item.id)} aria-label={en ? `Edit "${item.title}"` : `Editar "${item.title}"`} title={en ? "Edit quiz" : "Editar quiz"}>✎</button><button type="button" className={styles.deleteQuestion} onClick={() => deleteQuestion(item.id, item.title)} aria-label={en ? `Delete "${item.title}"` : `Excluir "${item.title}"`} title={en ? "Delete quiz" : "Excluir quiz"}>✕</button></div></div>; }) : <div className={styles.empty}>{en ? "There are no questions yet. Create the first one to begin." : "Ainda não há quizzes. Crie o primeiro para começar."}</div>}</div></section><section className={styles.dashboard} style={{ marginTop: 24 }}><header><div><span className={styles.kicker}>{en ? "TROPHY CHALLENGE HISTORY" : "HISTÓRICO DE DESAFIOS TROFÉU"}</span><h1>{en ? "Previous trophy challenges." : "Desafios troféu anteriores."}</h1><p>{en ? "Open a challenge's teacher panel to check scores or add its QR Code again." : "Abra o painel do professor de um desafio para conferir a pontuação ou pegar o QR Code de novo."}</p></div></header><div className={styles.list}>{challengeList.length ? challengeList.map((item) => <div className={styles.listItem} key={item.id}><a href={`?trofeu=${item.id}&painel=1`}><div className={styles.questionCardMain}><span className={styles.questionType}>{en ? "TROPHY CHALLENGE" : "DESAFIO TROFÉU"}</span><b>{item.title}</b><small>{en ? "Challenge" : "Desafio"} {item.id}</small></div><span className={styles.cardArrow} aria-hidden="true">→</span></a><div className={styles.cardActions}><button type="button" className={styles.editQuestion} onClick={() => startEditChallenge(item.id)} aria-label={en ? `Edit "${item.title}"` : `Editar "${item.title}"`} title={en ? "Edit challenge" : "Editar desafio"}>✎</button><button type="button" className={styles.deleteQuestion} onClick={() => deleteChallenge(item.id, item.title)} aria-label={en ? `Delete "${item.title}"` : `Excluir "${item.title}"`} title={en ? "Delete challenge" : "Excluir desafio"}>✕</button></div></div>) : <div className={styles.empty}>{en ? "No trophy challenges yet." : "Ainda não há desafios troféu."}</div>}</div></section></main>;
}
