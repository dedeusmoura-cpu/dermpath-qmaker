...
type Question = { id: number; title: string; stem: string; kind: "choice" | "open"; options: string[]; correctAnswer: number; imageUrl?: string | null };
...
function AppHeader({ home, onBack, language, setLanguage }: { home?: boolean; onBack?: () => void; language: Language; setLanguage: (language: Language) => void }) {
  return <header className={styles.appHeader}>
    <a className={styles.brand} href="/" aria-label="Ir para a página inicial do DermPath Quiz"><img src="/dermpath-quiz-logo.png" alt="DermPath Quiz" /><span><strong>DermPath <em>Quiz</em></strong><small>Votações clínicas</small></span></a>
...
export default function Home() {
  ...
  if (mode === "vote" && question) return <main className={styles.shell}><AppHeader language={language} setLanguage={setLanguage} onBack={goHome} /><section className={styles.card}><header><span className={styles.kicker}>{en ? "DERMPATH QUIZ" : "DERMPATH QUIZ"}</span><span>Dermatopatologia</span></header><article className={styles.question}><h1>{question.title}</h1><p>{question.stem}</p>{question.imageUrl && <img className={styles.micrograph} src={question.imageUrl} alt="Imagem da questão" />}{question.kind === "open" ? <textarea className={styles.openAnswer} maxLength={180} disabled={message.startsWith("Resposta")} value={openAnswer} onChange={(event) => setOpenAnswer(event.target.value)} placeholder={en ? "Write a short response…" : "Escreva uma resposta curta…"} /> : <div className={styles.choices}>{question.options.map((option, index) => <button disabled={message.startsWith("Resposta")} className={selected === index ? styles.selected : ""} onClick={() => { setSelected(index); setMessage(""); }} key={option}><b>{letters[index]}</b>{option}</button>)}</div>}<button className={styles.primary} onClick={vote}>{en ? "Submit answer" : "Confirmar resposta"}</button>{message && <p className={message.startsWith("Resposta") ? styles.success : styles.error}>{message}</p>}</article></section></main>;

  return <main className={styles.shell}><AppHeader home language={language} setLanguage={setLanguage} /><section className={styles.dashboard}><header><div><span className={styles.kicker}>DERMPATH QUIZ</span><h1>{en ? "Questions that engage the class." : "Questões que envolvem a turma."}</h1><p>{en ? "Create a question, project the QR code, and follow the distribution." : "Crie uma pergunta, projete o QR Code e acompanhe a distribuição."}</p></div><button className={styles.primary} onClick={() => setMode("new")}>{en ? "+ New question" : "+ Nova questão"}</button></header><div className={styles.list}>{questions.length ? questions.map((item) => <a key={item.id} href={`?q=${item.id}&painel=1`}><span className={styles.kicker}>{item.kind === "open" ? (en ? "OPEN RESPONSE" : "RESPOSTA ABERTA") : `${en ? "QUESTION" : "QUESTÃO"} ${item.id}`}</span><b>{item.title}</b><small>{en ? "Open panel and QR Code →" : "Abrir painel e QR Code →"}</small></a>) : <div className={styles.empty}>{en ? "There are no questions yet. Create the first one to begin." : "Ainda não há questões. Crie a primeira para começar."}</div>}</div></section></main>;
}
